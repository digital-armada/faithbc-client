...
import EventItem from "@/components/features/events/components/EventItem";
...
export default function Event({ initialEvents }) {
  ...
  const getEventDate = (event) => {
    try {
      return event?.attributes?.eventStartDate
        ? parseISO(event.attributes.eventStartDate)
        : null;
    } catch {
      return null;
    }
      ...
      <div className="container py-8">
        ...
        <div className="mt-8 space-y-12">
          {Object.entries(eventsByMonth).map(([monthYear, events]) => (
            <div key={monthYear} className="rounded-lg">
