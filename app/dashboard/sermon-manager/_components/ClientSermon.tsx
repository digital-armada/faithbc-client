...
export default function ClientSermon({ sermon, speakers, series }) {
  ...
  const handleConvertVideo = async () => {
    ...
    try {
      ...
      // Check if the result is successful and includes expected data
      if (
        result.success &&
        Array.isArray(result.data) &&
        result.data.length > 0
      ) {
        const fileId = result.data[0].id; // Assuming result contains an array with an 'id'
        setAudioFileId(fileId); // Update state with the file ID
        setValue("audio", fileId, { shouldValidate: true }); // Sync form data with new audio ID
      } else {
        console.error(
          "Conversion failed or data structure unexpected:",
          result.error || result,
        );
      }
    } catch (error) {
